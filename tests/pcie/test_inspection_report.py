...
class InspectionReportTests(unittest.TestCase):
    ...
    def test_renderers_share_json_and_html_escapes_evidence_text(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("## 先看重點", markdown)
            self.assertIn("不知道這份 trace", markdown)
            self.assertIn("技術附錄（需要重現或核對身份時再看）", markdown)
            self.assertLess(markdown.index("先看重點"), markdown.index("技術附錄"))
            self.assertNotIn("PASS — bounded", markdown)
            markdown_main, markdown_appendix = markdown.split("## 技術附錄", 1)
            self.assertNotIn("PASS_BOUNDED", markdown_main)
            self.assertIn("PASS_BOUNDED", markdown_appendix)
            self.assertIn("<title>PCIe Trace 檢視報告 — &lt;img", page)
            self.assertIn("這份報告檢查什麼？", page)
            self.assertIn("<summary>技術附錄：", page)
            html_main, html_appendix = page.split("<summary>技術附錄：", 1)
            self.assertNotIn("PASS_BOUNDED", html_main)
            self.assertIn("PASS_BOUNDED", html_appendix)
            ...
            self.assertIn("不代表裝置測試 PASS", page)
    ...
    def test_bundle_writes_one_structured_source_and_refuses_overwrite(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("前五筆", md_path.read_text(encoding="utf-8"))
            self.assertIn("僅前五筆", html_path.read_text(encoding="utf-8"))
