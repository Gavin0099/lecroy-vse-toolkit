# PCIe 候選問題位置報告（S0-Remove SD7-1350.pex）

工具在 `S0-Remove SD7-1350.pex` 的 938 筆 TLP 裡列出 16 個值得先看的位置。每一項都附上 LeCroy 可以直接跳過去的 packet 編號。位置依它們在 trace 裡出現的先後排列，順序不代表嚴重程度。

這份報告只整理工具觀察到的事。這份 trace 的測試結果還沒確認（ground truth `UNKNOWN`），報告沒有判斷哪一項和故障有關。
...
- `Segment`，同一個 finding 裡位置相近的一段 packet
...
`Packet 1944517`，這一項最早出現的 request。在 LeCroy 用 Go to Packet 跳過去。
...
- 這一項的 3 個 Segment 前後相隔約 149,084 packets。放在同一個 finding 是因為 evidence 有關聯，不代表這幾段屬於同一個故障事件。
...
`Packet 1944519`，這一項最早出現的 request。在 LeCroy 用 Go to Packet 跳過去。
...
- 這份 trace 的 ground truth 還沒確認（`UNKNOWN`），工具沒有判斷這裡是否異常。
- `SLOTPOWERLIMIT` 和 這裡的 request 目前只確認位置接近，沒有證明因果。判斷附近與否用的 64 packets 範圍，是看過資料後才訂的暫定條件。
- 這一項的 3 個 Segment 前後相隔約 149,096 packets。放在同一個 finding 是因為 evidence 有關聯，不代表這幾段屬於同一個故障事件。
- 同一組 key 再次出現，不代表 requester 重送、`Tag` 用法違規，或 Completion 落在 capture 範圍外。
...
`Packet 1944521`，這一項最早出現的 request。在 LeCroy 用 Go to Packet 跳過去。
...
- 這一項的 2 個 Segment 前後相隔約 148,567 packets。放在同一個 finding 是因為 evidence 有關聯，不代表這幾段屬於同一個故障事件。
- 同一組 key 再次出現，不代表 requester 重送、`Tag` 用法違規，或 Completion 落在 capture 範圍外。
...
`Packet 1944523`，這一項最早出現的 request。在 LeCroy 用 Go to Packet 跳過去。
...
- 這份 trace 的 ground truth 還沒確認（`UNKNOWN`），工具沒有判斷這裡是否異常。
- `SLOTPOWERLIMIT` 和 這裡的 request 目前只確認位置接近，沒有證明因果。判斷附近與否用的 64 packets 範圍，是看過資料後才訂的暫定條件。
- 這一項的 3 個 Segment 前後相隔約 149,129 packets。放在同一個 finding 是因為 evidence 有關聯，不代表這幾段屬於同一個故障事件。
- 同一組 key 再次出現，不代表 requester 重送、`Tag` 用法違規，或 Completion 落在 capture 範圍外。
...
`Packet 1944525`，這一項最早出現的 request。在 LeCroy 用 Go to Packet 跳過去。
...
- 這份 trace 的 ground truth 還沒確認（`UNKNOWN`），工具沒有判斷這裡是否異常。
- `ERR_COR`、`SLOTPOWERLIMIT` 和 `UR` 目前只確認位置接近，沒有證明因果。判斷附近與否用的 64 packets 範圍，是看過資料後才訂的暫定條件。
- 這一項的 2 個 Segment 前後相隔約 483 packets。放在同一個 finding 是因為 evidence 有關聯，不代表這幾段屬於同一個故障事件。
- 同一組 key 再次出現，不代表 requester 重送、`Tag` 用法違規，或 Completion 落在 capture 範圍外。
...
`Packet 1944527`，這一項最早出現的 request。在 LeCroy 用 Go to Packet 跳過去。
...
- 這份 trace 的 ground truth 還沒確認（`UNKNOWN`），工具沒有判斷這裡是否異常。
- `ERR_COR`、`SLOTPOWERLIMIT` 和 `UR` 目前只確認位置接近，沒有證明因果。判斷附近與否用的 64 packets 範圍，是看過資料後才訂的暫定條件。
- 這一項的 2 個 Segment 前後相隔約 486 packets。放在同一個 finding 是因為 evidence 有關聯，不代表這幾段屬於同一個故障事件。
...
`Packet 1944531`，這一項最早出現的 request。在 LeCroy 用 Go to Packet 跳過去。
...
- 這份 trace 的 ground truth 還沒確認（`UNKNOWN`），工具沒有判斷這裡是否異常。
- `ERR_COR`、`SLOTPOWERLIMIT` 和 `UR` 目前只確認位置接近，沒有證明因果。判斷附近與否用的 64 packets 範圍，是看過資料後才訂的暫定條件。
- 這一項的 2 個 Segment 前後相隔約 487 packets。放在同一個 finding 是因為 evidence 有關聯，不代表這幾段屬於同一個故障事件。
...
`Packet 1944533`，這一項最早出現的 request。在 LeCroy 用 Go to Packet 跳過去。
...
- 這份 trace 的 ground truth 還沒確認（`UNKNOWN`），工具沒有判斷這裡是否異常。
- `ERR_COR`、`SLOTPOWERLIMIT` 和 `UR` 目前只確認位置接近，沒有證明因果。判斷附近與否用的 64 packets 範圍，是看過資料後才訂的暫定條件。
- 這一項的 2 個 Segment 前後相隔約 490 packets。放在同一個 finding 是因為 evidence 有關聯，不代表這幾段屬於同一個故障事件。
...
`Packet 1944535`，這一項最早出現的 request。在 LeCroy 用 Go to Packet 跳過去。
...
- 這一項的 2 個 Segment 前後相隔約 148,530 packets。放在同一個 finding 是因為 evidence 有關聯，不代表這幾段屬於同一個故障事件。
...
`Packet 1944537`，這一項最早出現的 request。在 LeCroy 用 Go to Packet 跳過去。
...
- 這份 trace 的 ground truth 還沒確認（`UNKNOWN`），工具沒有判斷這裡是否異常。
- `ERR_COR` 和 `UR` 目前只確認位置接近，沒有證明因果。判斷附近與否用的 64 packets 範圍，是看過資料後才訂的暫定條件。
- 這一項的 2 個 Segment 前後相隔約 148,538 packets。放在同一個 finding 是因為 evidence 有關聯，不代表這幾段屬於同一個故障事件。
...
`Packet 1944539`，這一項最早出現的 request。在 LeCroy 用 Go to Packet 跳過去。
...
- 這份 trace 的 ground truth 還沒確認（`UNKNOWN`），工具沒有判斷這裡是否異常。
- `ERR_COR` 和 `UR` 目前只確認位置接近，沒有證明因果。判斷附近與否用的 64 packets 範圍，是看過資料後才訂的暫定條件。
- 這一項的 2 個 Segment 前後相隔約 148,542 packets。放在同一個 finding 是因為 evidence 有關聯，不代表這幾段屬於同一個故障事件。
...
`Packet 1944547`，這一項最早出現的 request。在 LeCroy 用 Go to Packet 跳過去。
...
- 這份 trace 的 ground truth 還沒確認（`UNKNOWN`），工具沒有判斷這裡是否異常。
- `ERR_COR` 和 `UR` 目前只確認位置接近，沒有證明因果。判斷附近與否用的 64 packets 範圍，是看過資料後才訂的暫定條件。
- 這一項的 2 個 Segment 前後相隔約 148,546 packets。放在同一個 finding 是因為 evidence 有關聯，不代表這幾段屬於同一個故障事件。
...
`Packet 1944549`，這一項最早出現的 request。在 LeCroy 用 Go to Packet 跳過去。
...
`Packet 1944553`，這一項最早出現的 request。在 LeCroy 用 Go to Packet 跳過去。
...
`Packet 1944555`，這一項最早出現的 request。在 LeCroy 用 Go to Packet 跳過去。
...
`Packet 1944557`，這一項最早出現的 request。在 LeCroy 用 Go to Packet 跳過去。
