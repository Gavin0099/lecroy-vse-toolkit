...
Add-Type -Path (Join-Path $PSScriptRoot 'm2-ltssm-extraction.cs')
$result = [M2LtssmRunner]::Run($TracePath, $ScriptPath)
[GC]::Collect()
...
$allEventTypesMatch = $false
if ($artifact.events.Count -gt 0) {
    $allEventTypesMatch = $true
    foreach ($event in $artifact.events) {
        if ($event.Type -ne '_USB3_LTSSM_STATE') {
            $allEventTypesMatch = $false
            break
        }
    }
...
[pscustomobject]@{
    ...
    all_event_types_match = $allEventTypesMatch
    trace_integrity_unchanged = $traceIntegrityUnchanged
