...
public sealed class M2LtssmEvent
{
...
public interface IM2LtssmEngineEvents
{
    [DispId(1)]
...
public sealed class M2LtssmEventSink : IM2LtssmEngineEvents
{
    ...
    public List<M2LtssmEvent> Events { get; } = new List<M2LtssmEvent>();
    ...
    public void OnNotifyClient(int eventId, object eventBody, int tag)
    {
        ...
        if (eventId == 2201)
        {
            ...
            if (marker != "M2-PACKET-EXTRACTION-2")
            {
                Error = "unexpected M2 LTSSM event marker: " + marker;
                return;
            }
            ...
            {
                Error = "LTSSM batch record count did not match payload count";
                return;
            }
            ...
            foreach (string record in records)
            {
                ...
                if (fields.Length != 3)
                {
                    Error = "LTSSM record did not contain index, timestamp, and type";
                    return;
                }

                Events.Add(new M2LtssmEvent
                {
        ...
        if (eventId == 2202)
        {
            if (values.Length != 2)
            {
                Error = "LTSSM summary payload did not contain 2 items";
                return;
            }

            string marker = Convert.ToString(values.GetValue(lower)) ?? "";
            if (marker != "M2-PACKET-EXTRACTION-2-SUMMARY")
            {
                Error = "unexpected M2 LTSSM summary marker: " + marker;
                return;
            }
...
public interface IM2LtssmConnectionPointContainer
{
    ...
    void FindConnectionPoint(
        ...
        [MarshalAs(UnmanagedType.Interface)] out IM2LtssmConnectionPoint connectionPoint);
}

[ComImport]
[Guid("B196B286-BAB4-101A-B69C-00AA00341D07")]
[InterfaceType(ComInterfaceType.InterfaceIsIUnknown)]
public interface IM2LtssmConnectionPoint
{
    void GetConnectionInterface(out Guid eventInterface);

    void GetConnectionPointContainer(
        [MarshalAs(UnmanagedType.Interface)] out IM2LtssmConnectionPointContainer container);
...
public sealed class M2LtssmResult
{
    ...
    public List<M2LtssmEvent> Events { get; set; } = new List<M2LtssmEvent>();
    public List<string> Reports { get; set; } = new List<string>();
    public string Error { get; set; } = "";
}

public static class M2LtssmRunner
{
    public static M2LtssmResult Run(string tracePath, string scriptPath)
    {
        var result = new M2LtssmResult();
        object analyzer = null;
        object trace = null;
        object engine = null;
        IM2LtssmConnectionPoint connectionPoint = null;
        int cookie = 0;
        bool advised = false;
        var sink = new M2LtssmEventSink();
        ...
        try
        {
            ...
            var container = (IM2LtssmConnectionPointContainer)engine;
            Guid eventInterface = typeof(IM2LtssmEngineEvents).GUID;
